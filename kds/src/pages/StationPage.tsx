  ...
  return (
    <div className="flex min-h-full flex-col gap-10 bg-canvas px-10 py-10">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl">Choose this screen's station</h1>
          <p className="mt-2 text-lg text-ink-soft">
            The choice is remembered — this display boots straight back into it.
          </p>
        </div>
        <button
          type="button"
      ...
      {failed !== null && (
        <p className="text-lg text-danger">
          {readErrorMessage(failed, 'Could not load stations. Check the connection.')}
        </p>
      )}
        ...
        {(modePin === null || modePin === 'counter') && (
          <section className="flex flex-col gap-4">
            ...
            {counters.data !== undefined && counters.data.length === 0 && (
              <p className="text-base text-ink-faint">No counters configured.</p>
            )}
          </section>
        )}
        ...
        {(modePin === null || modePin === 'kitchen') && (
          <section className="flex flex-col gap-4">
            ...
            {groups.data !== undefined && groups.data.length === 0 && (
              <p className="text-base text-ink-faint">No printing groups configured.</p>
            )}
          </section>
        )}
